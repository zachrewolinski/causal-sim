...
def any_nan_torch(x):
    return bool(torch.any(torch.isnan(x)).detach().cpu().numpy())
    ...
    class MLP(torch.nn.Module):
        ...
        def forward(self, causes=None):
            ...
            if causes is None:
                ...
                # print("shape of causes_w[0]", causes_w[0].shape)
                # print("shape of causes_w[1]", causes_w[1].shape)
                causes = torch.cat(causes_w, axis=2)
            ...
            #     print(f"BIAS of Layer {idx}:")
            #     if hasattr(self.layers[idx], 'bias'):
            #         print(self.layers[idx].bias)
            #     else:
            #         print(f"Layer {idx} does not have a bias attribute.")
            # print("line 276")
            ...
            if self.is_causal:
                ...
                # print("random_idx_y", random_idx_y)
                # print("type of random_idx_y", type(random_idx_y))
